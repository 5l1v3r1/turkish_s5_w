...
for x in train dev test; do 
  ...
  [ -s  data/$x/spk2utt ] && { utils/validate_data_dir.sh --no-feats data/$x || exit 1; }
done
...
# Amit: validate data/lang instead of data/lang_test_bg since they are identical except 
# that an extra file, G.fst, is present in data/lang_test_bg. Hence, copy G.fst from
# data/lang_test_bg to data/lang and then validate data/lang.
